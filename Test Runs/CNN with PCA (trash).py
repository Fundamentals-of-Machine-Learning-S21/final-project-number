...
def PrePro_PCA(X_PCA):
    ...
    Preprocessed_PCA_data = pca.fit_transform(X_PCA)
    
    return Preprocessed_PCA_data
...
Preprocessed_PCA_data = PrePro_PCA(Preprocessed_data)


print('pre-processed data shape', Preprocessed_PCA_data.shape)
print(labels.shape)
np.save('data_preprocessed_PCA', Preprocessed_PCA_data)


def load_data():
    data_preprocesssed_PCA = np.load('data_preprocessed_PCA.npy')
    labels = np.load('labels_train.npy')
    X = data_preprocesssed_PCA # (3360, 400)
    y = labels # (3360,)
