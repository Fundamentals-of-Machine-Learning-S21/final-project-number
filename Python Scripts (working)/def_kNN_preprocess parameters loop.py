# -*- coding: utf-8 -*-
"""
Created on Wed Mar 31 21:42:47 2021
...
K_range = range(35, 100, 5)
Resize_range = range(10, 40, 5)

all_kNN_Scores = np.zeros(1)
resize_kNN_Scores = np.zeros(1)
resize_kNN_Scores2 = np.zeros(len(Resize_range))
for i in K_range:
    ...
    for j in Resize_range:
        ...
        # Delete .T and column select if doing more than threshold 
        ...
        resize_kNN_Scores = np.vstack((resize_kNN_Scores, kNN_score))
    all_kNN_Scores = resize_kNN_Scores[1:,:]


np.reshape(all_kNN_Scores, ((len(K_range), len(Resize_range))))
TopScore = np.amax(all_kNN_Scores)
TopScore_loc = np.where(all_kNN_Scores == (np.amax(all_kNN_Scores)))

print('Scores: Kernel Ranges X Resize Ranges', all_kNN_Scores.shape)
print('Highest Accuracy', TopScore)
print('Highest Accuracy Location', TopScore_loc)


#WILL NEED TO RESHAPE (NP.RESHAPE) TO MAKE BY KERNE, BY RESIZE

# # RESIZE DIMENSIONS PARAMETER LOOP
# all_kNN_Scores = np.zeros(1)
...
#     all_kNN_Scores = np.vstack((all_kNN_Scores, kNN_score))
# all_kNN_Scores = all_kNN_Scores[1:,:]
